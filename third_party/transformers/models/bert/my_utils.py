...
def convert_subtok_to_tok(args, input_emb, input_mask):
    if (args.use_new_pooling == 'true'):
        #print ("Original", torch.count_nonzero(input_emb.sum(dim=-1),dim=-1))
        sum_sequence_output = torch.zeros_like(input_emb)
        expanded_pooling_ids = input_mask.unsqueeze(-1).expand(-1, -1, input_emb.size(-1))
        sum_sequence_output.scatter_add_(dim=1, index=expanded_pooling_ids, src=input_emb)

        #print ("First input", input_mask[0])
        #print ("Third input", input_mask[2])
        #print ("Output", sum_sequence_output.shape, torch.count_nonzero(sum_sequence_output.sum(dim=-1),dim=-1))
        out_tensor =sum_sequence_output
        out_no_extra = sum_sequence_output

...
UDPOS_LANGUAGES = 'zh vi ko ja'.split(' ')


#def update_vocab_dict(tokenizer, new_tokens):
#    #print ("Intersection", set(new_tokens).intersection(set(tokenizer.vocab.keys())))
#    #intersect = set(new_tokens).intersection(set(tokenizer.vocab.keys()))
#    #act_new_tokens = set(new_tokens).union(set(tokenizer.vocab.keys()))
#    #act_new_tokens = set(new_tokens).difference(intersect)
#    #tokenizer.add_tokens(list(act_new_tokens))
#    tokenizer.add_tokens(list(new_tokens))
#    print ("New length", len(tokenizer))
...
def panx_tokenize_preprocess(args):
  def _preprocess_one_file(infile, outfile, idxfile, tokenizer, max_len):
    ...
    with open(infile, "rt") as fin, open(outfile, "w") as fout, open(idxfile, "w") as fidx:
      for line in fin:
        ...
        ipa_subwords_len = len(tokenizer.tokenize(items[2].strip()))
        ...
        if (ipa_subwords_len == 0 or ipa_subwords_len > max_seq_len) and len(ipa) != 0:
          #print ("Unknown ipa case", ipa, ipa_subwords_len, tokenizer.unk_token)
          #print ("IPA check", prior_line, line, ipa, out_counter)
          ipa = tokenizer.unk_token
          ipa_subwords_len = 1

        len_ipa.append(ipa_subwords_len)
        len_word.append(current_subwords_len)

	
        if ((subword_len_counter + current_subwords_len) > max_seq_len) or((subword_len_ipa_counter + ipa_subwords_len) > max_seq_len) : #max_len obtained, remove the rest?
          fout.write(f"\n{token}\t{label}\t{ipa}\t{roman}\n")
          fidx.write(f"\n{idx}\n")

          subword_len_counter = current_subwords_len
          subword_len_ipa_counter = ipa_subwords_len
          ...
          fout.write(f"{token}\t{label}\t{ipa}\t{roman}\n")
          fidx.write(f"{idx}\n")
          out_counter += 1
          subword_len_counter += current_subwords_len
          subword_len_ipa_counter += ipa_subwords_len

    #print ("In-out", in_counter, out_counter)
    len_ipa, len_word = np.array(len_ipa), np.array(len_word)
    #print ("Check IPA", np.mean(len_ipa), np.std(len_ipa))
    #print ("Check word", np.mean(len_word), np.std(len_word))
  ...
  if (args.expand_vocab == 'true'):
    ...
    print ("updated tokenizer", len(tokenizer))
    #tokenizer = update_vocab_dict(tokenizer,ipa)
        ...
        if len(items) == 2:
          ...
          if idx != -1:
            ...
            fout.write(f'{token}\t{label}\n')
              ...
              print ("Fail cases", items)	
        else:
          print ("Not length 2 cases", items)
          fout.write('\n')
...
def pretrain_tokenize_preprocess(args):
  def _preprocess_one_file(infile, outfile, idxfile, tokenizer, max_len):
    if not os.path.exists(infile):
    ...
    with open(infile, "r", encoding='utf8') as fin:
      for line in fin:
        ...
        ipa_subwords_len = len(tokenizer.tokenize(items[2].strip()))
        if (current_subwords_len == 0 or current_subwords_len > max_seq_len) and len(token) != 0:
          print ("--Unknown word case")
          token = tokenizer.unk_token
          current_subwords_len = 1


        if (ipa_subwords_len == 0 or ipa_subwords_len > max_seq_len) and len(ipa) != 0:
          #print ("Unknown ipa case", tokenizer.unk_token)
          ipa = tokenizer.unk_token
          ipa_subwords_len = 1


        if ((subword_len_counter + current_subwords_len) > max_seq_len) or((subword_len_ipa_counter + ipa_subwords_len) > max_seq_len) : #max_len obtained, remove the rest?
          fout.write(f"\n{token}\t{label}\t{ipa}\t{roman}\t{lang_name}\n")
          fidx.write(f"\n{idx}\n")
          subword_len_counter = current_subwords_len
          subword_len_ipa_counter = ipa_subwords_len
          ...
          fout.write(f"{token}\t{label}\t{ipa}\t{roman}\t{lang_name}\n")
          fidx.write(f"{idx}\n")
          subword_len_counter += current_subwords_len
          subword_len_ipa_counter += ipa_subwords_len
          out_counter += 1
  ...
  if (args.expand_vocab == 'true'):
    ...
    for file in files:
      ...
      infile = os.path.join(args.data_dir, "{}.tsv".format(file))
      outfile = os.path.join(out_dir, "{}.{}".format(file, args.model_name_or_path))
      idxfile = os.path.join(out_dir, "{}.{}.idx".format(file, args.model_name_or_path))
      if os.path.exists(outfile) and os.path.exists(idxfile):
        print(f'{outfile} and {idxfile} exist')
...
def udpos_tokenize_preprocess(args):
  def _preprocess_one_file(infile, outfile, idxfile, tokenizer, max_len):
    ...
    fout = open(outfile, 'w')
    fidx = open(idxfile,'w')
    #with open(infile, "r", encoding='utf8') as fin, open(outfile, "w", encoding='utf8') as fout, open(idxfile, "w") as fidx:
    ...
    with open(infile, "r", encoding='utf8') as fin:
      for line in fin:
        ...
        current_subwords_len = len(tokenizer.tokenize(token))
        ipa_subwords_len = len(tokenizer.tokenize(items[2].strip()))
        #print ("Check token ipa", token, ipa, tokenizer.tokenize(token), tokenizer.tokenize(ipa))
        if (current_subwords_len == 0 or current_subwords_len > max_seq_len) and len(token) != 0:
          print ("--Unknown word case")
          token = tokenizer.unk_token
          current_subwords_len = 1
        ...
        #current_subwords_len = max(current_subwords_len, ipa_subwords_len)
...
if (True):
  ...
  parser.add_argument("--expand_vocab", default="false", type=str,
                      help="vocab expansion")
  ...
  if args.task == 'pretrain_tokenize':
    #print ("PRETRAIN-TOK")
    pretrain_tokenize_preprocess(args)
  if args.task == 'udpos':
